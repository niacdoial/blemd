...
class Material:
    def __init__(self, mbase, mat3):



        for i in range(mat3.tevCounts[mbase.tevCountIndex]):
            order = mat3.tevOrderInfos[mbase.tevOrderInfo[i]];
            stage = mat3.tevStageInfos[mbase.tevStageInfo[i]];
            ...
            alphaIns = ['']*4
            alphaIns[0] = getAlphaIn(stage.alphaIn[0], mbase.constAlphaSel[i], order)
            alphaIns[1] = getAlphaIn(stage.alphaIn[1], mbase.constAlphaSel[i], order)
            alphaIns[2] = getAlphaIn(stage.alphaIn[2], mbase.constAlphaSel[i], order)
            alphaIns[3] = getAlphaIn(stage.alphaIn[3], mbase.constAlphaSel[i], order)
            alphaIns = ''.join(alphaIns)

            out << getOp(stage.alphaOp, stage.alphaBias, stage.alphaScale,
            stage.alphaClamp, stage.alphaRegId, alphaIns, 1) << "\n";
        ac = mat3.alphaCompares[mbase.alphaCompIndex]
        self.alcompared_a = getAlphaCompare(ac.comp0, ac.ref0)
        self.alcompared_b = getAlphaCompare(ac.comp1, ac.ref1)
        self.alcomparemethod = ac.alphaOp  # 0: and, 1:or, 2:Xor, 3:XNor
...
def readMat3Header(f, h):
    f.ReadFixedLengthString(h.tag)
    f.ReadDWORD(h.sizeOfSection)
    f.ReadWORD(h.count)
    f.ReadWORD(h.pad)
    for i in range(30):
        f.ReadDWORD(h.offsets[i])
...
def readMatIndirectTexturingEntry(f, indEntry):
    ...
    for k in range(3):
        ...
        indEntry.unk2[k].b = f.ReadFixedLengthString()
...
def dumpMat3(f, dst):
    ...
    if h.count != dst.stringtable.size():
        print("mat3: number of strings (%d) doesn't match number of elements (%d)",
              dst.stringtable.size(), h.count, file=sys.stderr)
    ...
    dst.materialbases.resize(maxIndex + 1)
    ...
    dst.tevCounts = [f.GetByte() for _ in lengths[19]]
